...
export const generateInvoicePDF = (invoice: Invoice) => {
  ...
  let yPos = 20;
  const purchasePaidAmount = invoice.purchasePricePaidAmount ?? 0;
  const isPurchasePricePaid = purchasePaidAmount > 0;
  ...
  autoTable(doc, {
    ...
    body: tableData,
    foot: (() => {
      const purchasePaid = purchasePaidAmount;
      const balanceDue = invoice.total - purchasePaid;
      const purchasePaidLabel = invoice.purchasePricePaidSourceLabel
        ? `Purchase Price Paid (${invoice.purchasePricePaidSourceLabel})`
        : 'Purchase Price Paid';
      return [
        ...
        ...(purchasePaid > 0 ? [
          ['', '', '', { content: purchasePaidLabel, styles: { textColor: COLORS.success } }, { content: `-${formatCurrency(purchasePaid)}`, styles: { textColor: COLORS.success } }],
