...
function parseNotificationsPayload(payload: unknown): { items: Notification[]; unread: number } {
  if (Array.isArray(payload)) {
    const items = payload.filter(isNotification);
    return { items, unread: items.filter((n) => !n.read).length };
  }

  if (payload && typeof payload === 'object') {
    const shaped = payload as NotificationListResponse;
    const items = Array.isArray(shaped.data) ? shaped.data.filter(isNotification) : [];
    const unread = typeof shaped.unreadCount === 'number'
      ? shaped.unreadCount
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Session may be unavailable during transitions; avoid surfacing as runtime errors.
        setNotifications([]);
        setUnreadCount(0);
      } else {
        setNotifications([]);
      ...
      // Ignore transient network failures (dev server restart, route transitions, SSE reconnects).
    } finally {
      setLoading(false);
    }
  }, []);
