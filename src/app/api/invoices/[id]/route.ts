...
function resolvePurchasePaymentSource(entry: {
  type: 'DEBIT' | 'CREDIT';
  transactionInfoType: 'CAR_PAYMENT' | 'SHIPPING_PAYMENT' | 'STORAGE_PAYMENT' | null;
  metadata: unknown;
}) {
  ...
  if (metadata.paymentCategory === 'PURCHASE_PRICE' && entry.transactionInfoType !== 'CAR_PAYMENT') {
    return 'SHIPMENT_PAYMENT_RECORD';
  }

  if (entry.transactionInfoType === 'CAR_PAYMENT') {
    return 'CAR_PAYMENT_RECORD';
  }
...
export async function GET(
  ...
  try {
    ...
    const purchaseLedgerTotals = purchaseLedgerEntries.reduce(
      (totals, entry) => {
    ...
    const purchasePaymentSources = new Set<string>();
    for (const entry of purchaseLedgerEntries) {
      if (!isPurchaseLedgerEntry(entry)) {
        continue;
      }

      const source = resolvePurchasePaymentSource(entry);
      if (source) {
        purchasePaymentSources.add(source);
      }
    }
    ...
    let purchasePricePaidSourceLabel: string | null = null;
    if (isPurchasePriceFullyPaid) {
      if (purchasePaymentSources.has('CAR_PAYMENT_RECORD') && purchasePaymentSources.has('SHIPMENT_PAYMENT_RECORD')) {
        purchasePricePaidSourceLabel = 'Car/Shipment payment record';
      } else if (purchasePaymentSources.has('CAR_PAYMENT_RECORD')) {
        purchasePricePaidSourceLabel = 'Car payment record';
      } else if (purchasePaymentSources.has('SHIPMENT_PAYMENT_RECORD')) {
        purchasePricePaidSourceLabel = 'Shipment payment record';
      }
    } else if (hasRecordedPayment) {
      purchasePricePaidSourceLabel = 'Recorded payment';
    }
