...
export async function PATCH(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // ⚡ Bolt: Replaced O(N) sequential updates and creates inside the loop with O(1) bulk updateMany and createMany operations.
      ...
      const auditLogs = charges.map((charge) => ({
        ...
        newValue: body.status!, // guaranteed by check above
        metadata: {
          mode: 'bulk',
