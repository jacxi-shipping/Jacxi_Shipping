...
export async function PATCH(
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // ⚡ Bolt: Compress O(N) sequential update and create queries inside the transaction into O(1) bulk operations
      const chargeIdsToUpdate = charges.map((charge) => charge.id);

      await tx.shipmentCharge.updateMany({
        where: { id: { in: chargeIdsToUpdate } },
        data: {
          status: body.status,
