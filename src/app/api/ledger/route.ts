...
export async function GET(request: NextRequest) {
  try {
    ...
    // ⚡ Bolt: Consolidated multiple separate .aggregate queries into a single .groupBy query
    const [totalCount, entries, groupedTotals, latestEntry] = await Promise.all([
      // Get total count
    ...
    const totalDebit = groupedTotals.find(g => g.type === 'DEBIT')?._sum?.amount || 0;
    const totalCredit = groupedTotals.find(g => g.type === 'CREDIT')?._sum?.amount || 0;
