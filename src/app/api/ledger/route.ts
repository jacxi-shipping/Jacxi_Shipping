...
export async function GET(request: NextRequest) {
  try {
    ...
    const filteredRunningBalanceByEntryId = new Map<string, number>();
    const filteredEntriesAscending = [...visibleEntries].sort((a, b) => {
      const dateDiff = new Date(a.transactionDate).getTime() - new Date(b.transactionDate).getTime();
      if (dateDiff !== 0) return dateDiff;
      return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
    });

    let filteredRunningBalance = 0;
    for (const entry of filteredEntriesAscending) {
      filteredRunningBalance += entry.type === 'DEBIT' ? entry.amount : -entry.amount;
      filteredRunningBalanceByEntryId.set(entry.id, filteredRunningBalance);
    }

    const visibleEntriesWithFilteredBalance = visibleEntries.map((entry) => ({
      ...entry,
      balance: filteredRunningBalanceByEntryId.get(entry.id) ?? entry.balance,
    }));

    const totalCount = visibleEntriesWithFilteredBalance.length;
    const pagedEntries = visibleEntriesWithFilteredBalance.slice((page - 1) * limit, page * limit);
