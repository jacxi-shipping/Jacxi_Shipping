...
export async function GET(
  ...
  try {
    ...
    // ⚡ Bolt: Replace chained .filter().reduce() with a single pass loop for O(N) performance
    ...
    const portalLedgerSummary = {
      ...
      creditAmount: portalCreditAmount,
    ...
    for (const entry of filteredPortalLedgerEntries) {
      if (entry.type === 'DEBIT') activityDebitAmount += entry.amount;
      else if (entry.type === 'CREDIT') activityCreditAmount += entry.amount;
    }
    ...
    // ⚡ Bolt: Replace chained .filter().reduce() and multiple .filter().length with a single pass loop for O(N) performance
