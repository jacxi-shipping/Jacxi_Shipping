...
export async function GET(
  ...
  try {
    ...
    // ⚡ Bolt: Replace chained array operations (.filter().reduce()) with single-pass loops
    // to improve performance by reducing O(2N) iteration to O(N) and eliminating
    // intermediate array allocations during portal ledger summary calculation.
    let portalTotalDebit = 0;
    let portalTotalCredit = 0;
    for (const entry of portalLedgerEntries) {
      if (entry.type === 'DEBIT') {
        portalTotalDebit += entry.amount;
      } else if (entry.type === 'CREDIT') {
        portalTotalCredit += entry.amount;
      }
    }

    const portalLedgerSummary = {
      balance: portalLedgerEntries[0]?.balance || 0,
      debitAmount: portalTotalDebit,
      creditAmount: portalTotalCredit,
    ...
    let activityTotalDebit = 0;
    let activityTotalCredit = 0;
    for (const entry of filteredPortalLedgerEntries) {
      if (entry.type === 'DEBIT') {
        activityTotalDebit += entry.amount;
      } else if (entry.type === 'CREDIT') {
        activityTotalCredit += entry.amount;
      }
    }

    const activitySummary = {
      debitAmount: activityTotalDebit,
      creditAmount: activityTotalCredit,
    ...
    // ⚡ Bolt: Replace multiple chained .filter().length / .filter().reduce() passes
    // with a single O(N) loop computing all aggregates simultaneously, saving memory
    // and significantly reducing CPU cycles for accounts with many invoices.
    ...
    for (const charge of unbilledCharges) {
      unbilledAmount += charge.totalAmount;
    }
