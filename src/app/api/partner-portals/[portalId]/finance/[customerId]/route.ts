...
export async function GET(
  ...
  try {
    ...
    // ⚡ Bolt: Replaced chained .filter().reduce() with a single pass loop for O(N) performance
    let portalLedgerSummaryDebit = 0;
    let portalLedgerSummaryCredit = 0;
    for (const entry of portalLedgerEntries) {
      if (entry.type === 'DEBIT') {
        portalLedgerSummaryDebit += entry.amount;
      } else if (entry.type === 'CREDIT') {
        portalLedgerSummaryCredit += entry.amount;
      }
    }

    const portalLedgerSummary = {
      balance: portalLedgerEntries[0]?.balance || 0,
      debitAmount: portalLedgerSummaryDebit,
      creditAmount: portalLedgerSummaryCredit,
    ...
    // ⚡ Bolt: Replaced chained .filter().reduce() with a single pass loop for O(N) performance
    let activitySummaryDebit = 0;
    let activitySummaryCredit = 0;
    for (const entry of filteredPortalLedgerEntries) {
      if (entry.type === 'DEBIT') {
        activitySummaryDebit += entry.amount;
      } else if (entry.type === 'CREDIT') {
        activitySummaryCredit += entry.amount;
      }
    }

    const activitySummary = {
      debitAmount: activitySummaryDebit,
      creditAmount: activitySummaryCredit,
