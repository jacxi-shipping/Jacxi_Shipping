...
export async function POST(request: NextRequest) {
  try {
    ...
    // ⚡ Bolt: Process photo downloads concurrently in batches of 10 to resolve N+1 network latency bottleneck
    const BATCH_SIZE = 10;
    for (let i = 0; i < photos.length; i += BATCH_SIZE) {
      const batch = photos.slice(i, i + BATCH_SIZE);
      await Promise.all(
        batch.map(async (photoUrl: string, indexInBatch: number) => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              const photoFilename = urlParts[urlParts.length - 1] || `photo-${absoluteIndex + 1}.jpg`;

              folder?.file(photoFilename, buffer);
            }
          } catch (error) {
            console.error(`Error downloading photo ${absoluteIndex}:`, error);
