...
interface Invoice {
	...
	discount: number;
	total: number;
	purchasePricePaidAmount?: number;
	purchasePricePaidSourceLabel?: string | null;
...
export default function InvoiceDetailPage() {
	...
	const getLineItemTypeLabel = (item: LineItem) => {
		...
		const expenseTypeFromDescription = extractExpenseTypeLabel(item.description);
		if (expenseTypeFromDescription) {
			return expenseTypeFromDescription;
		}
		return item.type.replace(/_/g, ' ');
	};

	const normalizeExpenseTypeLabel = (value: string) => {
		...
		return expenseTypeMap[normalized] ?? value.trim().replace(/_/g, ' ').toUpperCase();
	};

	const extractExpenseTypeLabel = (description: string): string | null => {
		const prefixedMatch = description.match(/(?:container|dispatch|transit)\s+expense\s*-\s*([A-Za-z_ ]+?)\s+for/i);
		if (prefixedMatch?.[1]) {
			return normalizeExpenseTypeLabel(prefixedMatch[1]);
		}

		const genericMatch = description.match(/\s-\s*([A-Za-z_ ]+?)\s+for\s+/i);
		if (genericMatch?.[1]) {
			return normalizeExpenseTypeLabel(genericMatch[1]);
		}
	...
	const purchasePaid = invoice.purchasePricePaidAmount ?? 0;
	const isPurchasePricePaid = purchasePaid > 0;
							...
							>
								<TableHead>
									<TableRow>
										...
										<TableCell sx={{ fontWeight: 600 }} align="center">Purchase Price Status</TableCell>
											...
											{group.items.map((item) => (
													...
													<TableCell align="left" sx={{ pl: 2, pr: 2 }}>
														<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', gap: 1, flexWrap: 'wrap', textAlign: 'left' }}>
															...
															{(item.type === 'PURCHASE_PRICE' || item.type === 'VEHICLE_PRICE') &&
																isPurchasePricePaid &&
																invoice.purchasePricePaidSourceLabel && (
																<Chip
																	label={`Paid by ${invoice.purchasePricePaidSourceLabel}`}
																	size="small"
																	variant="outlined"
																	sx={{ fontSize: '0.65rem' }}
																/>
													...
													<TableCell align="center">
														{item.type === 'PURCHASE_PRICE' || item.type === 'VEHICLE_PRICE' ? (
															<Chip
																label={isPurchasePricePaid ? 'Paid' : 'Not Paid'}
																size="small"
																color={isPurchasePricePaid ? 'success' : 'error'}
																variant={isPurchasePricePaid ? 'filled' : 'outlined'}
																sx={{ fontSize: '0.7rem' }}
															/>
														) : (
															<Box component="span" sx={{ color: 'var(--text-secondary)', fontSize: '0.75rem' }}>-</Box>
														)}
													</TableCell>
