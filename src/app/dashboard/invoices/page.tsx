...
interface InvoiceTableRow {
	...
	status: string;
	total: number;
	purchasePriceStatus: 'PAID' | 'NOT_PAID';
}
...
export default function InvoicesPage() {
	...
	const invoiceRows: InvoiceTableRow[] = filteredInvoices.map((invoice) => {
		...
		return {
			...
			total: invoice.total,
			purchasePriceStatus: (invoice.purchasePricePaidAmount ?? 0) > 0 ? 'PAID' : 'NOT_PAID',
		};
	});
	...
	const invoiceColumns: Column<InvoiceTableRow>[] = [
		...
		{
			key: 'purchasePriceStatus',
			header: 'Purchase Price',
			render: (value) => (
				<Chip
					label={value === 'PAID' ? 'Paid' : 'Not Paid'}
					size="small"
		...
		try {
			exportToCSVWithHeaders(
				rows.map((row) => ({
					...
					status: row.status,
					purchasePriceStatus: row.purchasePriceStatus === 'PAID' ? 'Paid' : 'Not Paid',
				...
				[
					...
					{ key: 'purchasePriceStatus', label: 'Purchase Price' },
							...
							<Table size="small">
								...
								<TableBody>
									{filteredInvoices.map((invoice) => {
										...
										const purchasePricePaid = (invoice.purchasePricePaidAmount ?? 0) > 0;
										return (
											<TableRow key={invoice.id} hover>
												...
												<TableCell>
													<Chip
														label={purchasePricePaid ? 'Paid' : 'Not Paid'}
														size="small"
														color={purchasePricePaid ? 'success' : 'error'}
														variant={purchasePricePaid ? 'filled' : 'outlined'}
														sx={{ fontSize: '0.75rem', minWidth: 88 }}
													/>
												</TableCell>
