...
export default function RecordPaymentPage() {
  ...
  const fetchUserShipments = async (userId: string) => {
    try {
      ...
      while (hasMore) {
        const response = await fetch(`/api/shipments?userId=${userId}&page=${page}&limit=${pageSize}&includeFinancial=true`);
        if (!response.ok) {
          break;
        }

        const data = await response.json();
        const pageShipments = (data.shipments || []) as Shipment[];
        allShipments = [...allShipments, ...pageShipments];
        const totalPages = data.pagination?.totalPages || 1;
        hasMore = page < totalPages;
        page += 1;
      }

      const dueShipments = allShipments
          .filter(
            (s) =>
              ...
              (
                (s.amountDue || 0) > 0 ||
                (s.purchaseAmountDue || 0) > 0 ||
                (s.expenseAmountDue || 0) > 0
              )
          );
            ...
            >
              {loadingShipments ? (
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  {(() => {
                    const q = normalizeSearchValue(shipmentSearch.trim());
                    const filtered = q
                      ? shipments.filter(
                          (s) => {
                            const vin = normalizeSearchValue(s.vehicleVIN);
                            const lot = normalizeSearchValue(s.lotNumber);
                            const vehicle = normalizeSearchValue(`${s.vehicleMake || ''}${s.vehicleModel || ''}`);
                            return (vin.includes(q) || lot.includes(q) || vehicle.includes(q)) && getShipmentDueForCategory(s) > 0;
                          }
                        )
