...
## 2026-06-10 - O(N*M) Chained Array Methods inside Loops
**Learning:** Found an instance in `src/app/api/users/[id]/route.ts` where generating user statement data chained multiple `.filter().reduce()` operations inside the statement building logic, processing an array multiple times for each variable.
**Action:** When aggregating multiple values (e.g. outstanding, overdue, paid amounts and counts) from the same list of objects in Node.js, replace chained `.filter().reduce()` or `.filter().length` with a single-pass O(N) `for...of` loop to avoid intermediate memory allocations and decrease CPU cycle overhead.
