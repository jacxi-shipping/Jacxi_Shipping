...
## 2024-05-18 - Ledger Payment O(N * M) Bottleneck
**Learning:** Found a textbook O(N * M) loop bottleneck in `src/app/api/ledger/payment/route.ts` where the code was calling `.reduce()` iteratively on a potentially large dataset of ledger entries for each shipment.
**Action:** Replace nested loops performing repeated array filtering or reduction with an O(N) pre-calculation pass using `Map`s for O(1) loop-ups.
