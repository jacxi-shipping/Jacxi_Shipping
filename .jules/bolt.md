...
## 2026-07-14 - Replaced O(N) queries with bulk operations for Shipment Charges
**Learning:** Sequential Prisma queries inside a loop in a transaction (like in `src/app/api/shipments/[id]/charges/route.ts`) create N+1 performance bottlenecks.
**Action:** Use `prisma.model.updateMany` and `prisma.model.createMany` instead of iterating and awaiting single queries to improve database throughput for bulk operations.
