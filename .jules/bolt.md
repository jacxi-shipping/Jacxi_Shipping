...
## 2024-05-24 - Single Pass Array Iterations
**Learning:** Found multiple instances where the codebase chained `.filter()` and `.reduce()` on the same arrays. This results in multiple passes over the array and allocating intermediate memory for the filtered array, causing unnecessary O(N+M) complexity and memory bloat on large datasets in API endpoints.
**Action:** Replaced chained array methods with single-pass `.reduce()` loops using internal ternaries to perform the check and sum efficiently in a single operation, keeping it as O(N) complexity.
