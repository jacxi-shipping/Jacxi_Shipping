...
## 2026-07-27 - Array Operation Consolidation
**Learning:** Found multiple instances where large arrays were iterated multiple times via chained `.filter(condition).reduce(...)` method calls. This causes unnecessary (N*M)$ complexity and allocates intermediate arrays which increases GC pressure.
**Action:** Always prefer consolidating chained array filters and maps/reduces into a single-pass loop (like `for...of`) when calculating multiple sums or counts over the same dataset.
