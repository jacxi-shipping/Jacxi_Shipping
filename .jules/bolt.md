...
## 2026-06-11 - Consolidated chained array methods to single-pass loops
**Learning:** Replaced chained .filter().reduce() loops with single pass O(N) loops. This optimizes execution speed avoiding intermediate arrays and overhead when working with large data records.
**Action:** Always favor using single pass loops (.reduce() with internal conditionals) over chaining .filter() and .reduce() where appropriate to avoid O(N+M) complexity.
