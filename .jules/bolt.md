...
## 2026-07-10 - Replaced array allocations with single pass logic
**Learning:** Found multiple instances where large arrays were mapped and reduced 10+ times to calculate summaries, causing O(N*M) time complexity and memory thrashing.
**Action:** Replaced chained .filter().reduce() operations with single pass loops mutating primitive variables.
